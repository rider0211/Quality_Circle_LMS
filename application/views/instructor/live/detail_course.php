									...
									<li>
										...
										<?php foreach($libraries as $chapter):?>
											...
											<?php foreach($libraries as $page):?>
												<?php if($page->parent != 0 && $page->parent == $chapter->id):?>
													<label class="radioBox" style="margin-left:20px"><?php echo $page->title?>
														<input onclick="changeSelect(this,<?php echo $page->id?>,<?php echo $page->exam_id?>)" id="library<?php echo $page->id?>" type="radio" name="radio" value="<?php echo $page->file_path?>">
														<span class="checkmark"></span>
													</label>
												<?php endif;?>
											<?php endforeach;?>
	...
	function finish_course(id){
		swal({
			...
			buttons: true
				...
				.then((willDelete) => {
			if (willDelete) {
			location.href = "<?=base_url()?>instructor/live/view_course/"+id;
		}
	});
	}
	$(function(){
