	...
	jQuery(document).ready(function() { 	
		...
		var tbdata = $history_table.dataTable({
			...
			"ajax": {
	            "type": "POST",
	            "async": true,
				"url": "<?=base_url()?>admin/demand/getQuizHistoryList"+"?course_id="+course_id+"&user_id="+user_id,
				"data": {'type':'general'},		
				"dataSrc": "data",
				"dataType": "json",
				"cache":    false,
	        ...
	        "columnDefs": [
                {
                    ...
                    "targets": [6],
                    "createdCell": function (td, cellData, rowData, row, col) {
                        $(td).html('<a class="btn btn-default btn-sm" target="blank" href="<?= base_url()?>admin/demand/view_quiz_answers/'+rowData['group_id']+'/'+rowData['user_id']+'">View Quiz Answers</a>');
                        }                    
                }
                ],
	        "columns": [
