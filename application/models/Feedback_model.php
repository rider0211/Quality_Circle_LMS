...
class Feedback_model extends CI_Model
{
     ...
     */
    function send_new_message($sender_id, $receiver_id, $message, $message_id, $priority=0)
    {
        $this->db->trans_start();
        ...
        $insert_id = $this->db->insert('feedback_message', $insert);
        ...
        $notif_data['to_user_id'] = $receiver_id;
        $notif_data['notification_title'] = "Receive Message";
        $notif_data['notification_message'] = $message;
...
//        send_phpmail_message('wangyi_1986@hotmail.com' , 'Test PHPMailer Mail', 'Hi! <strong>Demo Name</strong>');

        $company_id = $this->session->get_userdata()['company_id'];
        $result = $this->db->get_where("settings_email_template", "action='feedback_received' and company_id = ".$company_id);
        $email_temple=$result->row();
        $content = $email_temple->message;

        $theme = $this->getThemeSetting();
        $sender = $this->getUserByID($sender_id);
        $receiver = $this->getUserByID($receiver_id);

        $field = array('{FIRSTNAME}','{LASTNAME}','{SENDER}','{MESSAGE}','{SITE_NAME}');
        $value = array($sender->first_name,$sender->last_name,$receiver->first_name.' '.$receiver->last_name, $message,$theme['site_name']);

        $content = str_replace($field,$value,$content);
        $content = $this->rebuild_html($content);
        @send_phpmail_message($receiver->email , $email_temple->subject, $content);
     ...
     * @return array
     */
    function get_inbox_message($receiver_id)
    {
        $sql = 'SELECT fm.*,fm.status as rd_status, us.* FROM feedback_message as fm, user as us WHERE fm.receiver_id=? and fm.sender_id=us.id ORDER BY fm.created_at DESC, fm.status DESC' ;
    ...
    function get_admin_inbox_message()
    {
        $sql = "SELECT fm.*,fm.status as rd_status, us.* FROM feedback_message as fm, user as us WHERE fm.receiver_id=us.id ORDER BY fm.created_at DESC, fm.status DESC" ;
    ...
    function get_inbox_news_nums($receiver_id)
    {
        $sql = 'SELECT count(*) as cnt FROM feedback_message  WHERE receiver_id = '.$receiver_id.' and status=0' ;
    ...
    function get_admin_inbox_news_nums($receiver_id)
    {
        $sql = "SELECT count(*) as cnt FROM feedback_message as fm, user as us WHERE fm.receiver_id=us.id and fm.status=0 " ;
    ...
    function get_sent_message($sender_id)
    {
        $sql = 'SELECT fm.*, us.* FROM feedback_message  as fm, user as us WHERE fm.receiver_id=us.id and fm.sender_id = ? ORDER BY fm.created_at DESC' ;
    ...
    function get_message_details($msg_id)
    {
        $sql = 'SELECT * FROM feedback_message WHERE message_id = ? ORDER BY created_at ASC ' ;
    ...
    function get_last_message_details($msg_id)
    {
        $sql = 'SELECT * FROM feedback_message WHERE message_id = ? ORDER BY created_at DESC LIMIT 1 ' ;
    ...
    function update_message_status($msg_id, $status = 1)
    {
        $this->db->where(array('message_id' => $msg_id));
        $this->db->update('feedback_message', array('status' => $status ));
